/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v2.0 GA - https://canvasjs.com/ 
...
CanvasJS.Chart = (function () {
	...
	Axis.prototype.createLabels = function () {
		...
		if (this.type === "axisX" && this.valueType === "dateTime" && !this.logarithmic) {
			...
			for (var j = firstBreakIndex, i = this.intervalStartPosition; i <= endPoint ; i = parseFloat((this.logarithmic && this.equidistantInterval ? i * Math.pow(this.logarithmBase, this.interval) : i + this.interval).toFixed(12))) {
	...
	Axis.prototype.renderLabelsTicksAndTitle = function () {
		...
		if (this.labelPlacement === "inside") {
			this.chart.addEventListener("dataAnimationIterationEnd", function () {
				for (i = 0; i < _this._labels.length; i++) {
					label = _this._labels[i];
					if (!(label.position < _this.viewportMinimum || label.position > _this.viewportMaximum || skipLabels && labelCount++ % skipStep !== 0 && _this.labelAutoFit && label.skipFlag)) {
						_this.ctx.save();
						_this.ctx.beginPath();
			...
			for (var i = 0; i < this._labels.length; i++) {
				label = this._labels[i];
				if (!(label.position < this.viewportMinimum || label.position > this.viewportMaximum || skipLabels && labelCount++ % skipStep !== 0 && this.labelAutoFit && label.skipFlag))
					label.textBlock.render(true);
			}
		}
	}
...
CanvasJS.Chart.version = "v2.0 GA";
