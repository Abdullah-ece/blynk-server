...
public final class EventDBDao {
    ...
    private static final String insertEvent =
            "INSERT INTO reporting_events (id, device_id, type, ts, event_hashcode, description, is_resolved) "
                    + "values (?, ?, ?, ?, ?, ?, ?)";
    ...
    private static final String resolveLogEvent =
            "ALTER TABLE reporting_events UPDATE is_resolved = 1, resolved_by = ?, resolved_comment = ?, resolved_at = now() where device_id = ? and id = ?";
    ...
    public List<LogEventDTO> getEvents(int deviceId, EventType eventType, long from, long to,
        ...
        String isResolvedSql = isResolved == null ? "" : " and is_resolved = ? ";

        String query = "select * from reporting_events where device_id = ? and ts BETWEEN ? and ?"
                + eventTypeSql
                + isResolvedSql
                + "order by COALESCE(resolved_at, ts) desc limit ?, ?";
    ...
    public boolean resolveEvent(ResolveEventDTO resolveEventDTO, String resolvedBy) throws Exception {
        return resolveEvent(resolveEventDTO.logEventId,
                resolveEventDTO.deviceId, resolvedBy, resolveEventDTO.resolveComment);
    }

    public boolean resolveEvent(long id, int deviceId, String resolvedBy, String comment) throws Exception {
        int result;
        try (Connection connection = ds.getConnection();
